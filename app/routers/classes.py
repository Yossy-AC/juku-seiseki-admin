...
async def list_classes(db: Session = Depends(get_db), _: None = Depends(require_auth)):
    """講座一覧（HTMX用）"""
...
async def get_class_students(class_id: str, db: Session = Depends(get_db), _: None = Depends(require_auth)):
